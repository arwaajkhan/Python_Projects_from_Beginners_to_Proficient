import requests
from flight_data import FlightData

TEQUILA_ENDPOINT = "https://api.tequila.kiwi.com"
TEQUILA_API_KEY = "YOUR_API"


class FlightSearch:

    def get_iatacode(self, city_name):
        location_endpoint = f"{TEQUILA_ENDPOINT}/locations/query"
        headers = {"apikey": TEQUILA_API_KEY}
        query = {"term": city_name, "location_types": "city"}
        response = requests.get(url=location_endpoint, headers=headers, params=query)
        result = response.json()
        return result["locations"][0]["code"]

    def check_flights(self, origin_city_code, destination_city, from_time, to_time):
        headers = {"apikey": TEQUILA_API_KEY}
        query = {
            "fly_from": origin_city_code,
            "fly_to": destination_city,
            "date_from": from_time,
            "date_to": to_time,
            "nights_in_dst_from": 7,  # min days you want to stay at the destination
            "nights_in_dst_to": 28,  # max
            "flight_type": "round",
            "one_for_city": 1,  # It returns the cheapest flights to every city covered by the "to" parameter.
            "max_stopovers": 0,  # max number of stopovers per itinerary. Use 'max_stopovers=0' for direct flights only.
            "curr": "GBP"  # use this parameter to change the currency in the response.
        }

        response = requests.get(url=f"{TEQUILA_ENDPOINT}/v2/search", headers=headers, params=query)

        try:
            data = response.json()["data"][0]
        except IndexError:
            print(f"No flights found for {destination_city}.")
            return None

        flight_data = FlightData(
            price=data["price"],
            origin_city=data["route"][0]["cityFrom"],
            origin_airport=data["route"][0]["flyFrom"],
            destination_city=data["route"][0]["cityTo"],
            destination_airport=data["route"][0]["flyTo"],
            out_date=data["route"][0]["local_departure"].split("T")[0],
            return_date=data["route"][1]["local_departure"].split("T")[0]
        )
        # print(f"{flight_data.destination_city}: £{flight_data.price}")
        return flight_data
