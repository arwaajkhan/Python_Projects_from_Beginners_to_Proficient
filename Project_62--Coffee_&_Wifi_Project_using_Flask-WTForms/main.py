...
class CafeForm(FlaskForm):
    ...
    Location_url = StringField('Cafe Location on Google Maps(URL)', validators=[URL()])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        with open('cafe-data.csv', mode='a', encoding='UTF-8', newline='') as data_file:
            data = csv.writer(data_file)
            upload_file = [form.cafe.data, form.Location_url.data, form.opening_time.data, form.closing_time.data,
                           form.coffee_rating.data, form.wifi_rating.data, form.power_rating.data]
            data.writerow(upload_file)
